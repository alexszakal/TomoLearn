...
void Gen1CT::measure(const std::vector<double>& angles, int raysPerPixel){
	...
	sinogramE = Eigen::MatrixXd::Zero(pixNum, numAngles);
...
void Gen1CT::displayMeasurement(){
	...
	double maxInt = sinogramE.maxCoeff();
	for(int i=0; i<pixNum; ++i){
		for(int j=0; j<numAngles; ++j){
			sinoImage(i,j) = static_cast<uint16_t>(sinogramE(i,j)/maxInt*65536);
		}
	}
